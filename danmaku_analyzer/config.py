...
class Settings(BaseSettings):
    ...
    MOE: float = Field(default=0.05, gt=0, lt=1, description="抽样误差", json_schema_extra={"reproducible": True})
    CONFIDENCE_LEVEL: float = Field(default=0.95, gt=0, lt=1, description="置信水平", json_schema_extra={"reproducible": True})
    ...
    TOP_N: int = Field(default=10, ge=1, description="频次排序采样时取前N条弹幕", json_schema_extra={"reproducible": True})
    ...
    SIGNIFICANCE_ALPHA: float = Field(default=0.05, gt=0, lt=1, description="显著性检验的 alpha 水平", json_schema_extra={"reproducible": True})
    ...
    CORPUS_MIN_VIDEOS_PER_PARTITION: int = Field(default=3, ge=2, description="语料库比较时每分区最少视频数，少于此不参与比较", json_schema_extra={"reproducible": True})
    ...
    MIN_SEGMENT_SAMPLES: int = Field(default=30, ge=1, description="每个动态分段最少弹幕数", json_schema_extra={"reproducible": True})

    ENABLE_LLM_ANALYSIS_REPORT: bool = Field(default=True, description="是否启用LLM分析报告生成", json_schema_extra={"reproducible": True})
    LLM_CONCURRENCY: int = Field(default=5, ge=1, description="LLM 并发调用上限", json_schema_extra={"reproducible": True})
    SCHEDULER_WORKERS: int = Field(default=2, ge=1, description="任务调度器并发工作线程数（compare 批量视频级并行）", json_schema_extra={"reproducible": True})
