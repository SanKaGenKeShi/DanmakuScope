...
def _is_reproducible(field_info) -> bool:
    extra = field_info.json_schema_extra
    return bool(isinstance(extra, dict) and extra.get("reproducible"))
...
def reproducible_config_snapshot() -> Dict:
    ...
    for cls, instance in ((Settings, get_settings()), (LLMSettings, get_llm_settings())):
        for name, field_info in cls.model_fields.items():
            if _is_reproducible(field_info):
...
def build_repro_manifest() -> Dict:
    """完整运行环境快照：解释器/平台/依赖版本/白名单配置；流水线无随机采样组件，
    LLM 随机性由 config_snapshot 中的温度参数刻画，故无独立随机种子字段"""
    return {
        ...
        "config_snapshot": reproducible_config_snapshot(),
    }


def write_repro_manifest(output_dir: str) -> str:
    filepath = os.path.join(output_dir, REPRO_MANIFEST_FILENAME)
    with open(filepath, 'w', encoding='utf-8') as f:
        json.dump(build_repro_manifest(), f, ensure_ascii=False, indent=2)
