...
| 学术产出 | 方法论描述自动生成（单视频 methodology.md / 语料库级 corpus_methodology.md）+ LaTeX 表格片段 / APA 统计文本导出（推断文本覆盖 KW/逐对 MWU/配对 Wilcoxon/note 注记，含检验轴措辞） |
...
- **分析状态**：`analysis_status` 区分 `ok`、`degraded`（部分请求失败）和 `failed`（无有效标签）。失败记录仍以权重 `0.2` 保留供复核，未知维度不进入标签分布；全部标签失败时保留原始弹幕与硬统计档案，CLI/TUI 明示失败。
